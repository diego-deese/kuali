...
class LocationService {
  ...
  async createLocation(
    ...
    try {
      const response = await this.api.post('/locations', { name })

      if (response.status === 200) {
        return { success: true, data: response.data.location as Location }
      }

      return {
        success: false,
        message: response.data.message || 'Error al crear un nuevo lugar',
        error: response.data.error || 'No se pudo crear el lugar',
      } as ResponseError
    } catch (error) {
      if (axios.isAxiosError(error)) {
        const errorResponse = error.response?.data as ResponseError
  ...
  async deleteLocation(
    ...
    try {
      ...
      return {
        ...
        error: response.data.error || 'No se pudo eliminar el lugar',
      } as ResponseError
    } catch (error) {
      if (axios.isAxiosError(error)) {
        const errorResponse = error.response?.data as ResponseError
